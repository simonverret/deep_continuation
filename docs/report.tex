...
However, with $T_\text{{ref}}$ fixed in this way, the space of functions $p(\omega)$ we have to search to find the one compatible with $\vec \Pi$ includes all $sp(s\omega)$. In other words, any function $sp(s\omega)$ has to be ``outputable''. Equation~\eqref{eq_fix_T} simply tells us how to adjust $s$ when using a temperature different from $T_{\text{ref}}$.

\paragraph*{Fixing $s$}
Fixing $s$ instead of $T$ narrows down the search from all possible $sp(s\omega)$ to a single scale $s$ per $p(\omega)$. One way to do so is to fix the second moment of $p(\omega)$, as shown in section~\ref{sec_data_rescaling}. In this work, we hypothesize that narrowing the search space with fixed $s$ is desirable, and so we focus on that case from here on.

With this in mind, we define \emph{temperature-agnostic analytic continuation} as the task of mapping the response function vector $\Pi_n = \Pi_n[sp(s\omega)](T/s)$, obtained for temperature $T/s$ and density function $sp(s\omega)$ at any~$s$, to a single \emph{reference} density $p(\omega)$, same for all $s$:
\begin{align}
