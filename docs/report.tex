...
\section{Theory of temperature-agnostic analytic continuation}
...
\end{align}
As considered in this work, the analytic continuation problem is the inversion of this equation. That is, recover a numerical estimate of $\text{Re}\{ \sigma(\omega) \}$ given a numerical estimate for $\Pi(i\omega_n)$.
...
\end{align}
These are natural dimensionless input $\vec\Pi$ and output $p(\omega)$ for the analytic continuation problem. We then rewrite \eqref{dimensionless} with the change of variable $\omega\rightarrow s\omega$
\begin{align}
\Pi_n
&=
...
\end{align}
We thus see that, when applied on $sp(s\omega)$---where $s$ fits the definition of a \emph{scale parameter}---the integral kernel depends on the ratio~$\frac{T}{s}$.
...
The above dimensionless formulation shows that there is a degenerancy between analytic continuation problems at different temperatures. We can make this degenerancy explicit by writting integral~\eqref{dimensionless_with_p1} as a functional $\Pi_n = \Pi_n[p(\omega)](T)$, such that lines~\eqref{dimensionless_with_p1} and~\eqref{dimensionless_with_p2} become 
\begin{align}
\Pi_n[p(\omega)](T) = \Pi_n[sp(s\omega)](T/s).
\label{degenerancy}
\end{align}
This means that the same response function components $\Pi_n$ can be found at different temperatures $T$ and $T/s$ for respective distributions $p(\omega)$ and $sp(s\omega)$, for any $s$. Thus, without the temperature $T$ or frequency scale $s$ specified (with $\vec \Pi$ only), the analytic continuation task is ambiguous (degenerate with respect to $s$). Thus, in order to have a well defined problem, either $T$ or $s$ must be fixed.

\paragraph*{Fixing $T$}
Working from a single temperature $T_\text{{ref}}$ and setting $s=T_\text{{ref}}/T$ in~\eqref{degenerancy}
\begin{align}
...
Also, there is freedom in the specific choice of reference $p(\omega)$ to represent all other $sp(s\omega)$, and this choice requires care (section~\ref{sec_data_rescaling}).


\subsection{Recovering conductivity}
\label{sec_usage}
Suppose we have access to such an algorithm, for example a neural network $p_{\vec\theta}(\omega,\vec\Pi)$ (with $\vec \theta$ denoting all adjustable parameters) which takes $\vec \Pi$ as an input and approximates a single reference $p(\omega)$ as an output,
\begin{align}
...
\subsection{Rescaling training data}
\label{sec_data_rescaling}
The above procedures depend on the existence of the neural network $p_{\vec \theta}(\omega, \vec \Pi)$. Training such neural networks requires training data, more specifically several thousand pairs of inputs $\vec \Pi$ and output targets $p(\omega)$. To get a temperature-agnostic neural networks, the targets must be reference distributions $p(\omega)$ only. Here we only explain how to remove any $sp(s\omega)$ from the training dataset to get only reference distributions $p(\omega)$.
...
Fixing $\mu_2$ have three noteworthy advantages. First, all reference distributions will have similar widths in the $\omega$ space; there will be no exessively narrow or wide $p(\omega)$. Second, the constant $\mu_2$ simplifies equation~\eqref{user_s} so that the user can recover $s$ with:
\begin{align}
s
&=
\sqrt{
...
\end{align}
And third, if the generation process for $p_0(\omega)$ allows one to control the second central moment of the distribution analytically, the correct scale can be enforced without numerical overhead (we did not take advantage of this in the current work).
...
\section{Temperature-agnostic neural networks}

\subsection{Discrete formulation}
\label{section_discrete}

Analytic continuation with neural networks is typically done with  vector-to-vector neural networks~\cite{Fournier2020,Xie2019,Yoon2018,Kades2019}. The output of the neural network is a vector $\hat{\vec p} = p_{\vec \theta}(\vec \Pi)$ targets the desired function $p(\omega)$ discretized on a given grid of real frequencies,
\begin{align}
\omega_m &=
\Delta\omega m.
...
\end{align}
One must be careful and remember that discretizing integrals like the above is only an approximation. 
As such, we recommended to avoid sums like ~\eqref{discretized_continuation}  whenever possible, and perform more accurate integration of $p(\omega)$.
Furthermore, seeing the kernel in~\eqref{discretized_continuation} as a matrix and inverting that matrix, although also ill-conditionned, is not the same as solving the continuous analytic continuation problem.
...
It is beneficial to make output components $p_m$ sum to one, in order to use a softmax output unit in the neural network. Considering the discretized normalization integral for $p(\omega)$,
\begin{align}
\int d\omega p(\omega) = 1 \approx \sum_m \Delta\omega p(\Delta\omega m),
\end{align}
we see that $p_m$ must indeed correspond to $[\vec p]_m = \Delta\omega p(\Delta\omega m)$.
...
\end{align}
Both $\vec p$ and $\vec p_s$ sum to one, because both $p(s)$ and $sp(s\omega)$ integrate to one. Note, however, that the sampling frequencies are not the same.
...
% \paragraph*{standardization} Input data is standardized.
% \paragraph*{architecture} Fully connected, ReLU, BatchNorm, Softmax output unit.
% \paragraph*{loss function} L1 loss function
% \paragraph*{optimizer} Adam, learning rate scheduler, learning rate warm-up.
% \paragraph*{hyper-parameters search}
...
Omitting the $A,B$ subscript for simplicity, we can cleverly rewrite it as
\begin{align}
...
\end{align}
Notably, the first term of \eqref{kramers_plus_imaginary} is one of the two \emph{Kramer-Kronig relations} (\emph{Hilbert transforms}):
\begin{align}
...
\end{align}
The last term vanishes because $\text{Re}\{\sigma(\omega)\}$ is even (not shown), which shows that $\Pi(i\omega_n)$ is real. Equation~\eqref{chkpt_Pi_continuation} is the same as~\eqref{Pi_continuation}, and hence our derivation is complete.
