...
You can use this python script to define hyperparameters in three ways:
    - Through argparse style arguments 
        ex: python cnn_ctmo.py --weight_decay 10
    - Trhough a json file named 'params.json' or custom name
        ex: python cnn_ctmo.py
        with: 'params.json' containing
            {
                "weight_decay":10,
                ...
            }
        or: python cnn_ctmo.py --file anyname.json
...
parser.add_argument('--file', type=str, default='cnn_params.json', help='defines default parameters from a .json file')
...
def name(args):
    name = 'cnn{}-{}-{}_bs{}_lr{}_wd{}_drop{}{}{}'.format(
...
class CNN(nn.Module):
    def __init__(self, args):
        ...
        self.conv = nn.Sequential(
            # Layer 1
            nn.Conv1d(in_channels=1, out_channels=4, kernel_size=33, padding=0),
            nn.Dropout(p=0.5),
            nn.ReLU(),
            # nn.MaxPool2d(kernel_size=(2, 2), stride=2),

            # Layer 2
            nn.Conv1d(in_channels=4, out_channels=8, kernel_size=33, padding=0),
            nn.Dropout(p=0.5),
            nn.ReLU(),
            # nn.MaxPool2d(kernel_size=(2, 2), stride=2),

            # Layer 3
            nn.Conv1d(in_channels=8, out_channels=16, kernel_size=33, padding=0),
            nn.Dropout(p=0.5),
            nn.ReLU(),
            # nn.MaxPool2d(kernel_size=(2, 2), stride=2),
        )
        ...
        # Linear output
        self.linear = nn.Linear(16*32, args.out_size)

    def forward(self, x):
        out = x.view(-1, 1, 128)
        out = self.conv(out)
        out = out.view(-1, 16*32)
        out = self.linear(out)
        return out
...
def train(args, device, train_loader, valid_loader): 
    cnn = CNN(args).to(device)
    cnn.apply(init_weights)

    optimizer = torch.optim.Adam(cnn.parameters(), lr = args.lr, weight_decay=args.weight_decay)
    ...
    with open('training_'+name(args)+'.csv', 'w' if args.overwrite else 'a') as f:
        ...
        for epoch in range(1,args.epochs+1):
            ...
            if args.save:
                torch.save(cnn.state_dict(), 'state_dict_'+name(args)+'_epoch{}_loss{:.9f}.pt'.format(epoch, avg_val_loss))
            ...
            if avg_val_loss < best_val_loss: 
                ...
                for filename in glob('BEST_loss*_epoch*'+name(args)+'*'):
                    os.remove(filename)
                torch.save(cnn.state_dict(), 'BEST_loss{:.9f}_epoch{}_'.format(avg_val_loss,epoch)+name(args)+'.pt')
            else: 
                early_stop_count -= 1
...
if __name__=="__main__":
    ...
    cnn = train(args, device, train_loader, valid_loader)
