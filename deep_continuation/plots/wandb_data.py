...
all_df = wdbu.download_wandb_table("deep_continuation/corrected_beta_scale")
...
fig = plt.figure(figsize=[7.5,3.7], dpi=80)
ax1 = plt.subplot(321)
...
N_df.groupby(["beta"]).agg({m:'min' for m in metrics['mae_FN']}).plot(kind='bar', ax=ax1, width=0.6, title=f"without rescaling", legend=False)
N_df.groupby(["beta"]).agg({m:'min' for m in metrics['mae_GN']}).plot(kind='bar', ax=ax2, width=0.6, legend=False)
N_df.groupby(["beta"]).agg({m:'min' for m in metrics['mae_BN']}).plot(kind='bar', ax=ax3, width=0.6, legend=False)
R_df.groupby(["beta"]).agg({m:'min' for m in metrics['mae_FR']}).plot(kind='bar', ax=ax4, width=0.6, title=f"with rescaling", legend=False)
R_df.groupby(["beta"]).agg({m:'min' for m in metrics['mae_GR']}).plot(kind='bar', ax=ax5, width=0.6, legend=False)
R_df.groupby(["beta"]).agg({m:'min' for m in metrics['mae_BR']}).plot(kind='bar', ax=ax6, width=0.6, legend=False)
...
ax1.grid(b=True, which="both", axis="y", lw=0.5)
ax2.grid(b=True, which="both", axis="y", lw=0.5)
ax3.grid(b=True, which="both", axis="y", lw=0.5)
ax4.grid(b=True, which="both", axis="y", lw=0.5)
ax5.grid(b=True, which="both", axis="y", lw=0.5)
ax6.grid(b=True, which="both", axis="y", lw=0.5)
...
# model_id = '1i41andr' # The chosen one
...
for i in range(Ni):
    for j in range(Nj):
        x = inputs.cpu()[i+Ni*j]
        t = targets.cpu()[i+Ni*j]
        y = outputs.cpu()[i+Ni*j]
        ax[i,j].plot(t.detach().numpy(), label="target")
        ax[i,j].plot(y.detach().numpy(), label="NN")
        ax[i,j].set_xticks([])
        ax[i,j].set_yticks([])
        ax[i,j].legend(frameon=False, handlelength=1)

        # ax[i,j].set_title(f"(MSE={mae_crit(y,t):4f}")
        # ax[i,j].set_xlabel('w')
...
#%% NEW SCALE FIGURE
import deep_continuation.function_generator as gen
